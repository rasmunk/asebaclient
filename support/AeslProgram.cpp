...
void AeslProgram::load(xmlDoc *doc)
{
		...
		for(int i = 0; i < (nodeset ? nodeset->nodeNr : 0); ++i) {
			...
			if(name && size) {
				...
				if(eventSize > ASEBA_MAX_EVENT_ARG_SIZE) {
					...
					loaded = false;
					break;
				} else commonDefinitions.events.push_back(Aseba::NamedValue(Aseba::UTF8ToWString((const char *) name), eventSize));
			}
			xmlFree(name);
			xmlFree(size);
		}
	...
	if((obj = xmlXPathEvalExpression(BAD_CAST"/network/constant", context))) {
		...
		for(int i = 0; i < (nodeset ? nodeset->nodeNr : 0); ++i) {
			...
			if(name && value) commonDefinitions.constants.push_back(Aseba::NamedValue(Aseba::UTF8ToWString((const char *) name), atoi((const char *) value)));
